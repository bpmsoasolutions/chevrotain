...
describe("Chevrotain's Parsing Exceptions", () => {
    ...
    describe("the mappings between a an exception instance and its matching an exception's name for: ", () => {
        ...
        it_node("EarlyExitException", () => {
            let exceptionInstance = new exceptions.EarlyExitException("", dummyToken)
            expect(exceptionInstance.name).to.equal(functionName(exceptionInstance))
        })

        it_node("NoViableAltException", () => {
            let exceptionInstance = new exceptions.NoViableAltException("", dummyToken)
            expect(exceptionInstance.name).to.equal(functionName(exceptionInstance))
        })

        it_node("NotAllInputParsedException", () => {
            let exceptionInstance = new exceptions.NotAllInputParsedException("", dummyToken)
            expect(exceptionInstance.name).to.equal(functionName(exceptionInstance))
        })

        it_node("MismatchedTokenException", () => {
            let exceptionInstance = new exceptions.MismatchedTokenException("", dummyToken)
            expect(exceptionInstance.name).to.equal(functionName(exceptionInstance))
        })

        // TODO: one for the recoveryException ? but is it even exposed?
    })
})
